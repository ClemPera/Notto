...
pub fn encrypt_note(title: String, content: String) -> Result<schema::Note, Box<dyn std::error::Error>> {
    ...
    let data_ser = serde_json::to_string(&data_clear)?;
    let ciphertext = cipher.encrypt(&nonce, data_ser.as_bytes()).unwrap();
...
pub fn decrypt_note(note: schema::Note) -> Result<NoteData, Box<dyn std::error::Error>> {
    ...
    let cipher = Aes256Gcm::new(&master_encryption_key);
    let plaintext = cipher.decrypt(&nonce, note.data.as_ref()).unwrap();
    let data_unser: NoteData = serde_json::from_slice(&plaintext)?;
