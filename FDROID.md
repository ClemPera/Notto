...
6. **Test a plain Gradle build (no Tauri CLI) locally before submitting.** F-Droid's build
   recipe calls `gradle assembleRelease` directly against `client/src-tauri/gen/android`,
   it does not run `tauri android build`. That matters because the Tauri CLI normally runs
   `npm run build` (producing `client/dist`) for you before invoking Gradle; a bare Gradle
   invocation won't do that. The Rust cross-compile itself should still work either way,
   since the `rust` Gradle plugin in `buildSrc` hooks into Gradle's task graph and invokes
   Cargo regardless of who started the build. To check locally:
   ```sh
   cd client && npm install && npm run build
   cd src-tauri/gen/android
   ./gradlew assembleRelease
   ```
   If that produces a working APK under `app/build/outputs/apk/release/`, the `subdir`
   layout is fine and the only thing the fdroiddata recipe needs is a `prebuild` step that
   runs the two `client` commands above before Gradle starts (see the draft below).
   There's also older history of Tauri Android tripping over F-Droid's build server
   replacing the `gradlew` wrapper it doesn't trust
   ([tauri-apps/tauri#6367](https://github.com/tauri-apps/tauri/issues/6367)), from a 2023
   alpha. Likely stale now that `gen/android` is committed as a plain Gradle project rather
   than generated by the CLI at build time, but worth actually running the fdroidserver
   build container before opening a merge request rather than assuming. Instructions are in
   the Quick Start Guide linked above.
...
- The `prebuild` step builds the frontend (`client/dist`) before Gradle runs, since
  fdroidserver calls Gradle directly rather than the Tauri CLI, which normally does this
  step for you. See item 6 above for how to test this exact sequence locally first.
